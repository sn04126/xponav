...
@section('content')
<h2 style="color: #1D5C3C; margin-bottom: 20px;">Edit Plan: {{ $plan->name }}</h2>
    ...
    <form action="{{ route('admin.plans.update', $plan) }}" method="POST">
        ...
        <div style="margin-bottom: 16px;">
            ...
            <textarea name="features" rows="5"
                      style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; resize: vertical;">{{ old('features', implode("\n", $features)) }}</textarea>
        </div>
        ...
        <div style="display: flex; gap: 10px;">
            <button type="submit" style="flex: 1; padding: 12px; background: #1D5C3C; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 15px; font-weight: 600;">Update Plan</button>
